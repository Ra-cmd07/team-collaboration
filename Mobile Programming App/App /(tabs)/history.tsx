  ...
  const exportToCSV = async () => {
    try {
      ...
      if (await Sharing.isAvailableAsync()) {
        ...
        await Share.share({ message: csvContent, title: 'Scan History CSV' });
      }
  ...
  const exportToJSON = async () => {
    try {
      ...
      if (await Sharing.isAvailableAsync()) {
        ...
        await Share.share({ message: jsonContent, title: 'Scan History JSON' });
      }
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: isDark ? '#0A0A1F' : colors.background }]}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        <Animated.View style={{ opacity: fadeAnim, transform: [{ scale: scaleAnim }] }}>
          <LinearGradient colors={isDark ? ['#2D1B69', '#581C87', '#2D1B69'] : ['#0F172A', '#581C87', '#0F172A']} style={styles.historyHeader}>
            ...
            <TouchableOpacity style={styles.historyHeaderContent} onPress={async () => { await Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); Alert.alert('Scan History', `Total of ${scans.length} security scans performed by ${user?.username}`); }} activeOpacity={0.8}>
...
const styles = StyleSheet.create({
  ...
  gridLine: { position: 'absolute', backgroundColor: 'rgba(168,85,247,0.08)', shadowColor: '#A855F7', shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.3, shadowRadius: 8 },
  gridLineVertical: { width: 1, height: 1000, top: -1000 },
  ...
  historyHeader: { borderRadius: moderateScale(20), padding: spacing.md, marginBottom: verticalSpacing.sm, borderWidth: 1, borderColor: 'rgba(168,85,247,0.4)', shadowColor: '#A855F7', shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.5, shadowRadius: 15, elevation: 8, overflow: 'hidden', position: 'relative' },
  waveBackground: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0 },
  lineGraphContainer: { position: 'absolute', width: '100%', height: 60, top: '20%', flexDirection: 'row', alignItems: 'flex-end' },
  lineSegment: { position: 'absolute', width: 8, backgroundColor: '#A855F7', borderTopLeftRadius: 2, borderTopRightRadius: 2, bottom: 0 },
  lineSegmentPink: { backgroundColor: '#EC4899' },
  lineSegmentViolet: { backgroundColor: '#8B5CF6' },
  historyHeaderContent: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: verticalSpacing.md, zIndex: 1 },
  ...
  filterButtonTextActive: { color: '#FFFF' },
  ...
  modalFooter: { flexDirection: 'row', padding: spacing.lg, borderTopWidth: 1, gap: spacing.md },
  modalDeleteButton: { flex: 1, flexDirection: 'row', backgroundColor: '#DC2626', paddingVertical: verticalScale(12), borderRadius: moderateScale(12), alignItems: 'center', justifyContent: 'center', gap: spacing.sm },
  modalDeleteIcon: { fontSize: normalize(18) },
  modalDeleteButtonText: { color: '#FFFFFF', fontSize: normalize(14), fontWeight: 'bold' },
  modalCloseButton: { flex: 1, paddingVertical: verticalScale(12), borderRadius: moderateScale(12), alignItems: 'center', justifyContent: 'center' },
  modalCloseButtonText: { fontSize: normalize(14), fontWeight: 'bold' },

  // Export Modal
  exportModalContent: { borderTopLeftRadius: moderateScale(24), borderTopRightRadius: moderateScale(24), paddingBottom: spacing.lg, ...shadows.lg },
  ...
  exportCancelButton: { marginHorizontal: spacing.lg, paddingVertical: verticalScale(14), borderRadius: moderateScale(12), alignItems: 'center', borderWidth: 1, borderColor: 'rgba(168,85,247,0.2)' },
